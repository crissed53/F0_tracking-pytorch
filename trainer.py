...
class F0TrackingModel(pl.LightningModule):
    ...
    def _step(self, batch, batch_idx, process='train'):
        hcqt = batch['hcqt']
        f0 = batch['f0']
        f0_hat = self.net(hcqt.float())
        loss = self.loss(f0, f0_hat)
        acc = self._calc_acc(f0, f0_hat)

        log = {f'{process}_loss': loss,
               f'{process}_acc': acc}
        output = {
            'progress_bar': {
        ...
        # train step requires additional 'loss' key
        if process == 'train':
            output['loss'] = loss
    ...
    def _save_model(self):
        ...
        model_name = f'model_{self.current_epoch:03d}.pt'
    ...
    def validation_epoch_end(self, outputs):
        val_loss = torch.stack([x['log']['val_loss'] for x in outputs]).mean()
        val_acc = np.mean([x['log']['val_acc'] for x in outputs])
